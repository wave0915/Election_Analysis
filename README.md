...
From the analysis above, we can provide a business proposal to the election commission on how this script can be used for any election. By using python, this script can be used with any election data with the header included "Ballot ID", "County" and "Candidate" in that order same as [election_results.csv](Resources/election_results.csv)'s header. In the header, there are three variables. Every ballot ID is unique, so we can discard it for consideration to make examples.  The first example to use the script is in a different state like Califonia. There would be different names like Alpine, Los Angeles, San Francisco, San Diego, etc in the County column. Although Califonia's county names are different from Colorado's, still the code will work by modifying the county names. And the second example would be using it in different election types like the state's governor election. Then, in the Candidate column, the candidate names would be like Jared Polis, Cary Kennedy, Mike Johnston, or Donna Lynne in Colorado. Although the example candidate names are different from ones in the given election data, we can use the code by modifying the candidate names. To conclude, this script can automate the process by using python in any district or any type of election since both "County" and "Candidate" columns can be modified. 
